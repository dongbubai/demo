...
public class ErmForCShareUtil {
	 ...
	 * 格式化金额
	 * @param uf
	 * @param pk_currency 币种PK
	 */
	public static UFDouble formatUFDouble(UFDouble uf, String pk_currency){
		uf = (uf == null) ? UFDouble.ZERO_DBL :uf;
		if(pk_currency != null){
//			uf = uf.setScale(power, UFDouble.ROUND_HALF_UP);
			uf = Currency.getFormaUfValue(pk_currency,uf);
		}else{
			uf = uf.setScale(2, UFDouble.ROUND_HALF_UP);
		}
		return uf;
	}
}